...
Growing up in this environment has motivated me to pursue a career in environmental data science, with the hope of making a meaningful impact on communities like mine that are affected by environmental issues.
...
leaflet() %>%
  ...
  addPolygons(data = wilmington,
              ...
              opacity = 1,
              weight = 3) %>%
  addEasyButton(easyButton(icon = "fa-home",
                           title = "Reset View",
                           onClick = JS("function(btn, map){ map.setView([33.7858, -118.2641], 12); }"))) %>%
  htmlwidgets::onRender("function(el, x) {el.style.width = '100%'; el.style.height = '450px';}")
```
...
captions <- c("Bren Hall",
              "MEDS Cohort 2025",
              "Field Trip to Dangermond Preserve",
              "Aloes in Wonderland",
              "Graduation",
              "Graffiti", 
              "Coffee Run", 
...
slickR(image_list,
       ...
       width = "350px",
       height = "400px") %synch%
  (slickR(captions, slideType = "p", width = "350px", height = "50px") + settings(arrows = FALSE)) +
